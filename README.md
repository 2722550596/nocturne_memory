...
2. **如果使用 SQLite**，系统会在应用迁移**之前**自动备份你的数据库文件（如 `your_db.db.20260303_143000.bak`）。**新建的空数据库会自动跳过此备份**（没有数据需要保护）。
3. 迁移完成后会在日志中输出确认信息。

> 备份文件保存在与数据库文件相同的目录下。如果迁移出现问题，你可以将 `.bak` 文件重命名回原文件名来恢复。
>
> 💡 如果你认为自动备份多余（数据库已支持审查/回退功能），可以在 Dashboard 的 **设置 → 开发者模式 / 高级** 中开启「跳过迁移自动备份」，或在 `config.json` 中设置 `"skip_migration_backup": true`。这只会跳过迁移前的文件备份，不影响审查/回退系统对数据变更的保护。
